...
export default function ResumeEditClient({
    ...
    try {
      ...
      if (updatedResume) {
        ...
        setTimeout(() => {
          ...
          router.push(`/jobs/${encodeURIComponent(job_id)}/resumes/${id}${query}`);
        }, 1500);
      } else {
  ...
  const handleCancel = () => {
    const params = searchParams.toString();
    const query = params ? `?${params}` : "";
    router.push(`/jobs/${encodeURIComponent(job_id)}/resumes/${resumeData.id}${query}`);
  };
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2">
            <FileText size={18} className="text-clay-600" />
