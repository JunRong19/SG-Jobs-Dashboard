...
export function SkillTag({ skill }: { skill: string }) {
  return (
    <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-clay-50 text-clay-700 border border-clay-100 transition-colors hover:bg-clay-100">
      {skill}
    </span>
  );
}
...
export function TimelineItem({
  ...
  return (
    <div className="relative pl-6 pb-6 last:pb-0 group">
      ...
      <div className="absolute left-[-3.5px] top-2 w-[8px] h-[8px] rounded-full bg-clay-500 border-2 border-white shadow-sm" />
...
export function FormField({
  ...
  const baseClasses =
    "w-full px-3 py-2 text-sm border border-gray-200 rounded-lg bg-gray-50/50 focus:bg-white focus:border-clay-400 focus:ring-1 focus:ring-clay-400/20 transition-all outline-none placeholder:text-gray-400";
...
export function AddItemButton({
  ...
  return (
    <button
      onClick={onClick}
      className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-clay-600 bg-clay-50 border border-clay-200 rounded-lg hover:bg-clay-100 transition-all"
    >
      <Plus size={13} /> {label}
    </button>
  );
